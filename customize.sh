...
if [ $? = 0 ];
	then
		...
		sed -i 's/tr.insertCell(-1).innerHTML\ \=\ info.leases\[i\].macaddr;/tr.insertCell(-1).innerHTML\ \=\ info.leases\[i\].macaddr;\ntr.insertCell(-1).innerHTML\ \=\ \x27<a\ target\="_blank"\ href\="http:\/\/macvendors.co\/results\/\x27\ +\ info.leases[i].macaddr\ +\ \x27">\x27\ +\ \x27line1<\/a>\ ,\ <a\ target\="_blank"\ href\="https:\/\/api.macvendors.com\/\x27\ +\ info.leases[i].macaddr\ +\ \x27">\x27\ +\ \x27line2<\/a>\x27;/g' openwrt/feeds/luci/modules/luci-mod-admin-full/luasrc/view/admin_status/index.htm
fi
